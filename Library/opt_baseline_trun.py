...
'''
class mgomp:
    def __init__(self,data,nboot,est_in,pi_in,G,t_last):
        # Main variables
    ...
    def calcs(self,tj):
        ...
        kappa= self.est[-1][1]
        tmp=kappa*tj
        cum_haz_aux=-(np.exp(gamma+tmp)-np.exp(gamma))/kappa
        cum_haz_aux_ast=-(np.exp(gamma+kappa*self.t_last)-np.exp(gamma))/kappa
        self.s[-1]=np.array([(np.exp(a)-np.exp(cum_haz_aux_ast))/(1.0-np.exp(cum_haz_aux_ast)) if a>udrfl else 0.0 for a in cum_haz_aux])
    # Density
    def caldensity(self,tj):
        ...
        kappa=self.est[-1][1]
        tmp=kappa*tj
        sf_aux=np.exp(-(np.exp(gamma+tmp)-np.exp(gamma))/kappa)
        sf_aux_ast=np.exp(-(np.exp(gamma+kappa*self.t_last)-np.exp(gamma))/kappa)
        comp_density_last=np.exp(gamma+tmp)*sf_aux/(1.0-sf_aux_ast)
        # Density comp and ghen
        self.comp_density[-1]=comp_density_last
    ...
    def evalfn(self,x,group):
        ...
        if group<self.G-1:
            ...
            cval=np.exp(gamma+tmp)
            hval=cval-np.exp(gamma)
            cval_ast=np.exp(gamma+kappa*self.t_last)
            hval_ast=cval_ast-np.exp(gamma)
            sf_aux_ast=np.exp(-(np.exp(gamma+kappa*self.t_last)-np.exp(gamma))/kappa)
            t1=sf_aux_ast*hval_ast/((1.0-sf_aux_ast)*kappa)
            tot1=np.sum(tau*(1.0-hval/kappa-t1))
            t1=sf_aux_ast*(-self.t_last*cval_ast/kappa+hval_ast/kappa**2)/(1.0-sf_aux_ast)
            aux=tj*(1.0-cval/kappa)
            tot2=np.sum(tau*(aux+hval/kappa**2+t1))
        fvec[0] = tot1
        fvec[1] = tot2
        return fvec
    ...
    def hazard(self,t):
        ...
        for i in range(self.G):
            self.weights[i]=(self.pi[i]*sf_comp[i])/(sf_general)
